...
export function HoldingSettingsForm({
  ...
  return (
    <div className="rounded-xl border border-neutral-800 p-5">
      <h2 className="text-sm font-medium text-neutral-300">Tax settings</h2>
      <p className="mt-1 text-sm text-neutral-500">
        Suggested withholding rate:{" "}
        {suggestedRate !== null
          ? formatRate(suggestedRate)
          : "no default for this domicile — set an override below"}
      </p>
      <form action={updateHoldingSettings} className="mt-4 flex flex-wrap items-end gap-4">
        ...
        <div className="flex flex-col gap-1">
          <label htmlFor="withholdingRateOverride" className="text-sm text-neutral-500">
            Override rate % (blank = use suggested)
          </label>
          <input
