...
export async function addTransaction(
  ...
  if (transactionType === "sell") {
    ...
    const mapped: HoldingTransaction[] = (existing ?? []).map((t) => ({
    ...
    if (quantity > quantityHeldAtDate) {
      return {
        error: `Can't sell ${quantity} shares — only ${quantityHeldAtDate} held as of that date`,
      };
    }
  }
  ...
  if (insertError) {
    return { error: "Something went wrong adding this transaction" };
  }
