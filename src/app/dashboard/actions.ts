...
export async function addHolding(
  ...
  if (!ticker || !Number.isFinite(quantity) || quantity <= 0) {
    return { error: "Ticker and a positive quantity are required" };
  }
  if (!acquiredDate) {
    return { error: "Date acquired is required" };
  }
  ...
  if (!overview) {
    return { error: "This ticker isn't supported yet", unsupportedTicker: ticker };
  }
  ...
  if (insertError) {
    if (insertError.code === "23505") {
      return { error: "You already have a holding for this ticker" };
    }
    return { error: "Something went wrong adding this holding" };
  }
  ...
  if (transactionError) {
    return { error: "Holding was created, but recording the initial transaction failed" };
  }
